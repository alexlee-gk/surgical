...
void Trajectory_Recorder::write_threads_to_file()
{
  ...
  Matrix3d start_rot;
  Matrix3d end_rot;
  _threads.front().get_thread_data(points, twist_angles, rest_lengths, depths);
 ...
 threadFile << (double)points.size() << "\n";
  ...
  {
    _threads[i].get_thread_data(points, twist_angles, rest_lengths, depths);
    ...
    {
      threadFile << points[j](0) << " " << points[j](1) << " " << points[j](2) << " " << twist_angles[j] << " " << rest_lengths[j] << " " << depths[j] << " ";
